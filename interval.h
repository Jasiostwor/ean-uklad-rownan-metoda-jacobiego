...
Interval<T> ISin(const Interval<T> &x) {
	...
	if (x.a > x.b)
		...
		do {
			...
			if ((w.a != 0) && (w.b != 0)) {
				if ((std::abs(w.a - w1.a) / std::abs(w.a) < eps)
						&& (std::abs(w.b - w1.b) / std::abs(w.b) < eps))
					finished = true;
				else
					;
			} else if ((w.a == 0) && (w.b != 0)) {
				if ((std::abs(w.a - w1.a) < eps)
						&& (std::abs(w.b - w1.b) / std::abs(w.b) < eps))
					finished = true;
				else
					;
				...
				if ((std::abs(w.a - w1.a) / std::abs(w.a) < eps)
						& (std::abs(w.b - w1.b) < eps))
					finished = true;
				else if ((std::abs(w.a - w1.a) < eps) & (std::abs(w.b - w1.b) < eps))
					finished = true;
			}
...
Interval<T> ICos(const Interval<T> &x) {
	...
	do {
		...
		if ((w.a != 0) && (w.b != 0)) {
			if (((std::abs(w.a - w1.a) / std::abs(w.a)) < 1e-18)
					&& (std::abs(w.b - w1.b) / std::abs(w.b) < 1e-18))
				finished = true;
		} else {
			if ((w.a == 0) && (w.b != 0)) {
				if ((std::abs(w.a - w1.a) < 1e-18)
						&& (std::abs(w.b - w1.b) / std::abs(w.b) < 1e-18)) {
					finished = true;
				}
			} else if (w.a != 0) {
				if ((std::abs(w.a - w1.a) / std::abs(w.a) < 1e-18)
						&& (std::abs(w.b - w1.b) < 1e-18))
					finished = true;
...
Interval<T> IExp(const Interval<T> &x) {
	...
	if ((x.a < 0) && (x.b > 0)) {
        Interval<T> ret; ret.a = 1; ret.b = 1; return ret;
    }
	if (x.a > x.b)
		st = 1;
	else {
		...
		do {
			...
			T currDiff = std::abs(oldMid - newMid);
...
inline Interval<T> operator *(int i, const Interval<T> &y) {
	Interval<T> x = { static_cast<T>(i), static_cast<T>(i) };
	switch (Interval<T>::mode) {
	case PINT_MODE:
		return IMul<T>(x, y);
	case DINT_MODE:
